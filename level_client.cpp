...
// BFS Parallel Traversal Function
std::vector<std::vector<std::string>> bfsParallel(CURL* curl, const std::string& start, int depth) {
  std::vector<std::vector<std::string>> levels;
  std::unordered_set<std::string> visited;
  std::mutex mtx;
  ...
  for (int d = 0; d < depth; d++) {
    ...
    if (num_nodes == 0) break;
      ...
      int NodesP = (num_nodes + num_threads - 1)/ num_threads;

      for (int t = 0; t < num_threads; t++) {
        int begin = t * NodesP;
        int end = std::min(begin + NodesP, num_nodes);

        threads.emplace_back([&, begin, end]() {
          CURL* thread_curl = curl_easy_init();

          for (int i = begin; i < end; i++) {
            std::string node = levels[d][i];

            try {
              auto neighbors = get_neighbors(fetch_neighbors(thread_curl, node));
              for (auto& neigh : neighbors) {
                ...
                std::cerr<<"Error while fetching neighbors of: "<<std::endl;
              }
          }
...
int main(int argc, char* argv[]) {
    ...
    for (const auto& n : bfsParallel(curl, start_node, depth)) {
